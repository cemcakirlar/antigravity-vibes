...
export default function BottleSelector({ selectedId, onSelect, compact = false }: BottleSelectorProps) {
    ...
    const getBottleInfo = (id: string) => {
        switch (id) {
            case 'pet-500':
                return { name: t('bottles.pet500.name'), desc: t('bottles.pet500.desc') };
            case 'pet-1500':
                return { name: t('bottles.pet1500.name'), desc: t('bottles.pet1500.desc') };
            case 'hospital-urinal':
                return { name: t('bottles.hospitalUrinal.name'), desc: t('bottles.hospitalUrinal.desc') };
            default:
                const bottle = BOTTLES.find(b => b.id === id);
                return { name: bottle?.name || '', desc: bottle?.description || '' };
        }
    };
    ...
    if (compact) {
        return (
            <div className="flex overflow-x-auto gap-3 pb-2 snap-x w-full">
                {BOTTLES.map((bottle) => {
                    const info = getBottleInfo(bottle.id);
                    return (
                        <button
                            key={bottle.id}
